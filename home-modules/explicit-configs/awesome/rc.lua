...
awful.screen.connect_for_each_screen(function(s)
    ...
    local tags = {
      names  = { "www", "term", "edt", "eda", "5", "6", "music", "todo", "com" },
      layouts = { layouts[1], layouts[1], layouts[1], layouts[2], layouts[1],
    ...
    -- Make reading
    -- comfortable on a wide monitor with the centerwork layout.
    s.tags[1].master_width_factor = 0.7
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Control" }, "e", function () awful.screen.focus_relative( 1) end, -- Possibly has conflicts
    ...
    awful.key({}, "XF86MonBrightnessUp", function ()
        awful.util.spawn("brightnessctl -c backlight s \"10%+\"")
    end),
    ...
    awful.key({}, "XF86AudioRaiseVolume", function ()
        awful.util.spawn("wpctl set-volume -l 1.0 @DEFAULT_AUDIO_SINK@ 5%+")
    end),
...
awful.rules.rules = {
    ...
    -- Add titlebars to dialogs (maybe they should be added to "normal" type clients as well)
    { rule_any = {
            type = { "dialog" },
            instance = { ".blueman-manager-wrapped" }
...
-- Move all clients to the correct tag when a screen is removed.
tag.connect_signal("request::screen", function(t)
    ...
    for s in screen do
        -- Find a screen that's not being deleted.
        if s ~= t.screen then
            -- Find a tag in said screen with the same name.
            local target_t = awful.tag.find_by_name(s, t.name)
            for _, c in ipairs(clients) do
                c:move_to_tag(target_t)
            end
            return
        end
    end
